...
apiready = function() {
    ...
    $('.btn-submit').click(function() {
        var userAccount = $("#accountName").val(); // 平台用户名
        ...
        var zhanghuImg = $("#pic0").val(); //账户图片base64
        if (isNullOrEmpty(zhanghuImg)) {
            tips("必须上传阿里巴巴账户图片");
            return false;
        }
        ...
        var caigouImg = $("#pic2").val(); //采购名片图片base64
        if (isNullOrEmpty(caigouImg)) {
            tips("必须上传采购名片图片");
            return false;
        }
        loading();
        api.ajax({
            ...
            data: {
                values: {
                    UserId: userId,
                    Token: userToKen,
                    PlatId: 4, //平台ID  1淘宝 2天猫 3京东 4阿里巴巴 5拼多多 7蘑菇街 6 美丽说
                    AccountId: accountId,
                    ...
                    Gender: nannv, //男女性别 √
                    Age: nianling, //年龄 √
                    UserCenterImg: zhanghuImg, //账号用户图片base64  √
                    AccountLevelImg: zhanghudengjiImg, //账户等级图片base64 √
                    VerifiedImg: caigouImg, //采购名片图片图片base64 √
                }
            }
        }, function(ret, err) {
            optionResult(ret);
...
function getBindInfo() {
    api.ajax({
        ...
        if (!isNullOrEmpty(JSON.stringify(ret))) {
            var json = $.parseJSON(JSON.stringify(ret));
            if (json.errcode == 0) {
                $("#albbName").val(json.obj.PlatAccount);
