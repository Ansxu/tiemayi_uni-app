...
$('.btn-submit').click(function() {
    var userAccount = $("#mgjName").val(); // 平台用户名
        ...
        api.ajax({
            ...
            data: {
                values: {
                    UserId: userId,
                    Token: userToKen,
                    PlatId: 7, //平台ID  1淘宝 2天猫 3京东 4阿里巴巴 5拼多多 6美丽说 7蘑菇街 8网易考拉
                    AccountId: accountId,
                    ...
                    UserInfoImg: zhanghujietuImg, //蘑菇街账户截图图片base64  √
                }
            }
        }, function(ret, err) {
            optionResult(ret);
...
function getBindInfo() {
    ...
    api.ajax({
        ...
        if (!isNullOrEmpty(JSON.stringify(ret))) {
            var json = $.parseJSON(JSON.stringify(ret));
            if (json.errcode == 0) {
                $("#mgjName").val(json.obj.PlatAccount);
